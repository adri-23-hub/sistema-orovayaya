...
export async function reportsRoutes(app: FastifyInstance) {
  ...
  // GET /v1/reports/ventas?fecha_inicio&fecha_fin&sucursal_id
  app.get("/v1/reports/ventas", { preHandler: [authenticate] }, async (request) => {
    ...
    if (sucursal_id) query = query.where(eq(ventas.sucursalId, sucursal_id));
    if (fecha_inicio) query = query.where(gte(ventas.createdAt, new Date(fecha_inicio)));
    if (fecha_fin) query = query.where(lte(ventas.createdAt, new Date(fecha_fin)));

    const rows = await query.orderBy(sql`${ventas.createdAt} DESC`);
